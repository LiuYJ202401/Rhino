...
| 输入 | `Plane plane` — 所在平面，`Interval domainU` — U 方向范围，`Interval domainV` — V 方向范围，`bool isPreview = false` |
| 输出 | `Brep` — 矩形平面 |
| 报错 | 平面无效或范围为零时返回 null |
| RhinoCommon | `NurbsSurface.CreateFromPlane(plane, u, v, 2, 2, 2, 2)` → `Brep.CreateFromSurface` |
...
| 输出 | `Brep` — 插值曲面 |
| 报错 | 点数 ≠ uCount × vCount 或网格过小时返回 null |
