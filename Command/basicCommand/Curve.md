...
| 约束 | 两点投影到平面 UV 后必须有 U 方向和 V 方向的宽度（任一方向为零则退化） |
| 报错 | 两点重合或两点在平面 UV 方向上投影重合时输出错误消息并返回 null |
...
| 输出 | `Polyline` — 矩形多段线（闭合） |
| 报错 | width/height ≤ 0 时输出错误消息并返回 null |
| RhinoCommon | 无直接构造，手动计算四角点 |
...
| 报错 | sides < 3 或 radius ≤ 0 时输出错误消息并返回 null |
...
| 输出 | `Polyline` — 多边形多段线（闭合） |
| 报错 | sides < 3 或两点重合时输出错误消息并返回 null |
...
| 报错 | sides < 3 或半径 ≤ 0 时输出错误消息并返回 null |
...
| 输出 | `Circle` — 圆对象 |
| 报错 | radius ≤ 0 时输出错误消息并返回 `Circle.Unset` |
...
| 输出 | `Circle` — 圆对象 |
| 报错 | radius ≤ 0 或 normal 为零时输出错误消息并返回 `Circle.Unset` |
...
| 输出 | `Circle` — 圆对象 |
| 报错 | 两点重合时输出错误消息并返回 `Circle.Unset` |
...
| 输出 | `Circle` — 圆对象 |
| 报错 | 三点共线时输出错误消息并返回 `Circle.Unset` |
...
| 输出 | `Circle` — 圆对象 |
| 报错 | 切向量为零或两点重合时输出错误消息并返回 `Circle.Unset` |
...
| 输出 | `Arc` — 圆弧对象 |
| 报错 | radius ≤ 0 或角度范围无效时输出错误消息并返回 `Arc.Unset` |
...
| 输出 | `Arc` — 圆弧对象 |
| 报错 | 方向为零向量或两点重合时输出错误消息并返回 `Arc.Unset` |
...
| 输出 | `NurbsCurve` — 椭圆曲线 |
| 报错 | radius1/radius2 ≤ 0 时输出错误消息并返回 null |
...
| 输出 | `NurbsCurve` — 椭圆曲线 |
| 报错 | 两点重合或 secondRadius ≤ 0 时输出错误消息并返回 null |
...
| 报错 | 控制点数 ≤ degree 或 degree < 1 时输出错误消息并返回 null |
...
| 输出 | `NurbsCurve` — NURBS 曲线 |
| 报错 | 节点数/权数不匹配、degree 无效、权重 ≤ 0 时输出错误消息并返回 null |
...
| 输出 | `NurbsCurve` — 插值曲线 |
| 报错 | 点数 < 2、degree 非奇数、切向量与周期冲突时输出错误消息并返回 null |
...
| 输出 | `NurbsCurve` — 拟合曲线 |
| 报错 | 点数 < degree+1 或 tolerance ≤ 0 时输出错误消息并返回 null |
...
| 输出 | `NurbsCurve` — 悬链线曲线 |
| 报错 | length < 两点距离或 gravity 为零向量时输出错误消息并返回 null |
...
| 输出 | `NurbsCurve` — 螺旋线曲线 |
| 报错 | turns ≤ 0 或半径 ≤ 0 时输出错误消息并返回 null |
...
| 报错 | rail 无效或参数无效时输出错误消息并返回 null |
...
| 报错 | segmentCount < 1 或曲线无效时输出错误消息并返回空列表 |
...
| 报错 | segmentLength ≤ 0 或曲线无效时输出错误消息并返回空列表 |
...
| 输入 | `Curve curve` — 目标曲线 |
| 输出 | `Point3d` — 起点 |
| 报错 | 曲线无效时输出错误消息并返回 `Point3d.Unset` |
...
| 输出 | `Curve[]` — 投影后的曲线数组 |
| 报错 | curves 或 target 为 null 时输出错误消息并返回空数组 |
...
| 输出 | `Curve[]` — 拉到曲面上的曲线数组 |
| 报错 | curves 为空或 target 无效时输出错误消息并返回空数组 |
...
| 输出 | `Curve[]` — 等高线数组 |
| 报错 | geometry 为 null 或 interval ≤ 0 时输出错误消息并返回空数组 |
